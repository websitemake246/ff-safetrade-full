...
router.get('/mine', tokenAuth, (req, res) => {
  const rows = db('SELECT * FROM listings WHERE user_id = ?').all(req.user.id);
...
router.put('/:id', tokenAuth, (req, res) => {
  const { title, platform, description, price_kobo, evidence_urls } = req.body || {};
  const listing = db('SELECT * FROM listings WHERE id = ?').get(req.params.id);
...
router.delete('/:id', tokenAuth, (req, res) => {
  ...
  db('DELETE FROM listings WHERE id = ?').run(req.params.id);
  res.json({ ok: true });
});
...
router.get('/:id', (req, res) => {
  ...
  db('UPDATE listings SET view_count = view_count + 1 WHERE id = ?').run(req.params.id);
  
  const seller = db('SELECT username, verification_status FROM users WHERE id = ?').get(listing.user_id);
