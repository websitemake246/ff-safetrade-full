...
router.get('/stats', tokenAuth, requireAdmin, (req, res) => {
  const totalUsers = db("SELECT COUNT(*) as count FROM users WHERE role = 'user'").get().count;
  const totalDeals = db("SELECT COUNT(*) as count FROM deals").get().count;
  const openListings = db("SELECT COUNT(*) as count FROM listings WHERE status = 'open'").get().count;
  const pendingDeals = db("SELECT COUNT(*) as count FROM deals WHERE status = 'pending_payment'").get().count;
  const activeDeals = db("SELECT COUNT(*) as count FROM deals WHERE status IN ('payment_confirmed', 'accounts_shared', 'verified')").get().count;
  const completedDeals = db("SELECT COUNT(*) as count FROM deals WHERE status = 'completed'").get().count;
  const disputedDeals = db("SELECT COUNT(*) as count FROM deals WHERE status = 'disputed'").get().count;
  
  const feesResult = db("SELECT SUM(middleman_fee_kobo) as total FROM deals WHERE status = 'completed'").get();
  const feesCollected = feesResult.total || 0;
  
  const volumeResult = db("SELECT SUM(amount_kobo) as total FROM deals WHERE status = 'completed'").get();
...
router.get('/users', tokenAuth, requireAdmin, (req, res) => {
  const rows = db("SELECT id, username, email, phone, full_name, role, verification_status, is_verified, created_at FROM users ORDER BY created_at DESC").all();
  res.json(rows);
});
...
router.get('/deals/:id', tokenAuth, requireAdmin, (req, res) => {
  const deal = db('SELECT * FROM deals WHERE id = ?').get(req.params.id);
  if (!deal) return res.status(404).json({ error: 'Deal not found' });
  
  const listing = db('SELECT * FROM listings WHERE id = ?').get(deal.listing_id);
  const buyer = db('SELECT id, username, email, phone FROM users WHERE id = ?').get(deal.buyer_id);
  const seller = db('SELECT id, username, email, phone, bank_name, account_number, account_name FROM users WHERE id = ?').get(deal.seller_id);
...
router.post('/deals/:id/verify', tokenAuth, requireAdmin, (req, res) => {
  ...
  db("UPDATE deals SET status = 'verified', updated_at = ? WHERE id = ?")
    .run(new Date().toISOString(), deal.id);

  db("INSERT INTO notifications (user_id, type, title, body) VALUES (?, ?, ?, ?)")
...
router.post('/deals/:id/reject', tokenAuth, requireAdmin, (req, res) => {
  ...
  db("UPDATE deals SET status = 'payment_confirmed', seller_account_details = NULL, updated_at = ? WHERE id = ?")
    .run(new Date().toISOString(), deal.id);
...
router.post('/deals/:id/payout', tokenAuth, requireAdmin, (req, res) => {
  ...
  const seller = db('SELECT bank_name, account_number, account_name FROM users WHERE id = ?').get(deal.seller_id);
  
  // Log payout
  db("INSERT INTO activity_logs (admin_id, action, target_type, target_id, details) VALUES (?, ?, ?, ?, ?)")
...
router.get('/disputes/:id', tokenAuth, requireAdmin, (req, res) => {
  const dispute = db('SELECT * FROM disputes WHERE id = ?').get(req.params.id);
  if (!dispute) return res.status(404).json({ error: 'Dispute not found' });
  
  const messages = db('SELECT dm.*, u.username as user_username, u.role as user_role FROM dispute_messages dm JOIN users u ON dm.user_id = u.id WHERE dm.dispute_id = ? ORDER BY dm.created_at ASC').all(dispute.id);
  const deal = db('SELECT * FROM deals WHERE id = ?').get(dispute.deal_id);
...
router.post('/disputes/:id/message', tokenAuth, requireAdmin, (req, res) => {
  ...
  const msgId = uuidv4();
  db('INSERT INTO dispute_messages (id, dispute_id, user_id, message, attachment_url) VALUES (?, ?, ?, ?, ?)')
...
router.post('/disputes/:id/resolve', tokenAuth, requireAdmin, (req, res) => {
  ...
  db("UPDATE disputes SET status = 'resolved', resolution = ?, resolved_at = ?, updated_at = ? WHERE id = ?")
    .run(resolution || 'Resolved by admin', new Date().toISOString(), new Date().toISOString(), dispute.id);
  
  db("UPDATE deals SET status = ?, updated_at = ? WHERE id = ?")
