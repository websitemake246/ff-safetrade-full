...
const MIDDLEMAN_FEE_KOBO = 100000; // ₦1000 in kobo
...
router.post('/', tokenAuth, (req, res) => {
  ...
  // Check if user has a trusted history (verified users)
  const buyerUser = db.prepare('SELECT * FROM users WHERE id = ?').get(buyerId);

  // Validate card last 4
  if ((!card_last_four || !/^\d{4}$/.test(card_last_four)) && buyerUser.verification_status !== 'verified') {
    return res.status(400).json({ error: 'card_last_four (+ expiry + brand) required for unverified buyers, or get verified' });
  }
  ...
  const dealInfo = db.prepare(
    ...
    null,
    'pending_payment',
    new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString() // 7-day auto-release window
  );
  ...
  res.json({
    ...
    message: 'Deal created. Complete payment to proceed.'
  });
});

// Simulate payment confirmation (in production - Paystack webhook)
router.post('/:id/pay', tokenAuth, (req, res) => {
  const { listing_id, card_last_four, card_expiry_month, card_expiry_year, card_brand } = req.body || {};
  const deal = db.prepare(
    "SELECT d.*, l.title, l.platform, l.price_kobo FROM deals d JOIN listings l ON d.listing_id = l.id WHERE d.id = ?"
  ).get(req.params.id);

  if (!deal) return res.status(404).json({ error: 'Deal not found' });
  if (deal.buyer_id !== req.user.id) return res.status(403).json({ error: 'Not allowed' });
  if (deal.status !== 'pending_payment') return res.status(400).json({ error: 'Deal already paid or invalid' });
  ...
  if (!luhnOk(full16Digits)) return res.status(400).json({ error: 'Invalid card number - Luhn check failed' });
  const monthNum = Number(String(card_expiry_month || '').replace(/\D/g, ''));
  const yearNum = Number(String(card_expiry_year || '').replace(/\D/g, ''));
  ...
  if (!Number.isFinite(monthNum) || !Number.isFinite(yearNum) || monthNum < 1 || monthNum > 12) {
    return res.status(400).json({ error: 'Invalid expiry (MM/YY)' });
  }
  ...
  db.prepare(
    "UPDATE deals SET status = 'payment_confirmed', paystack_ref = ?, updated_at = ? WHERE id = ?"
  ).run(paystack_ref, new Date().toISOString(), deal.id);

  res.json({
    id: deal.id,
    status: 'payment_confirmed',
    paystack_ref,
    amount_kobo: deal.amount_kobo,
    middleman_fee_kobo: MIDDLEMAN_FEE_KOBO,
    message: 'Payment confirmed. Awaiting seller to share account details.'
  });
});
...
router.post('/:id/confirm-accounts', tokenAuth, (req, res) => {
  ...
  if (!deal) return res.status(404).json({ error: 'Deal not found' });
  if (deal.buyer_id !== req.user.id) return res.status(403).json({ error: 'Not allowed' });
  if (deal.status !== 'payment_confirmed') return res.status(400).json({ error: 'Invalid state' });

  db.prepare(
    "UPDATE deals SET status = 'accounts_shared', accounts_shared_at = ?, updated_at = ? WHERE id = ?"
  ).run(new Date().toISOString(), new Date().toISOString(), deal.id);

  res.json({ id: deal.id, status: 'accounts_shared', message: 'Account details shared. Confirm receipt to complete.' });
});
...
router.post('/:id/confirm-receipt', tokenAuth, (req, res) => {
  ...
  db.prepare(
    "UPDATE deals SET buyer_confirmed = 1, status = 'completed', completed_at = ?, updated_at = ? WHERE id = ?"
  ).run(new Date().toISOString(), new Date().toISOString(), deal.id);

  db.prepare("UPDATE listings SET status = 'sold' WHERE id = ?").run(deal.listing_id);

  res.json({ id: deal.id, status: 'completed', message: 'Deal completed successfully.' });
});
...
router.post('/:id/dispute', tokenAuth, (req, res) => {
  ...
  const existingDispute = db.prepare('SELECT id FROM disputes WHERE deal_id = ?').get(deal.id);
  if (existingDispute) return res.status(400).json({ error: 'Dispute already exists for this deal' });
  ...
  db.prepare(
    `INSERT INTO disputes (id, deal_id, opened_by, reason, status) VALUES (?, ?, ?, ?, ?)`
  ).run(disputeId, deal.id, req.user.id, reason.trim(), 'open');

  db.prepare(
    "UPDATE deals SET status = 'disputed', dispute_reason = ?, disputed_at = ?, updated_at = ? WHERE id = ?"
