...
router.post('/', tokenAuth, (req, res) => {
  ...
  const listing = db("SELECT l.*, u.id as seller_id FROM listings l JOIN users u ON l.user_id = u.id WHERE l.id = ? AND l.status = 'open'").get(listing_id);
  ...
  db('INSERT INTO deals (id, listing_id, buyer_id, seller_id, amount_kobo, middleman_fee_kobo, status) VALUES (?, ?, ?, ?, ?, ?, ?)')
...
router.post('/:id/pay', tokenAuth, (req, res) => {
  ...
  db("UPDATE deals SET status = 'payment_confirmed', paystack_ref = ?, updated_at = ? WHERE id = ?")
...
router.post('/:id/upload-credentials', tokenAuth, (req, res) => {
  ...
  db("UPDATE deals SET seller_account_details = ?, status = 'accounts_shared', accounts_shared_at = ?, updated_at = ? WHERE id = ?")
    .run(JSON.stringify(credentials), new Date().toISOString(), new Date().toISOString(), deal.id);

  // Notify admin for verification
  const admins = db("SELECT * FROM users WHERE role = 'admin'").all();
...
router.post('/:id/confirm', tokenAuth, (req, res) => {
  ...
  db("UPDATE deals SET status = 'completed', buyer_confirmed = 1, completed_at = ?, updated_at = ? WHERE id = ?")
...
router.post('/:id/dispute', tokenAuth, (req, res) => {
  ...
  db('INSERT INTO disputes (id, deal_id, opened_by, reason, status) VALUES (?, ?, ?, ?, ?)')
    .run(disputeId, deal.id, req.user.id, reason.trim(), 'open');
  
  db("UPDATE deals SET status = 'disputed', dispute_reason = ?, disputed_at = ?, updated_at = ? WHERE id = ?")
